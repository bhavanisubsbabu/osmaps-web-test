...
###	This Scenario is already covered by 'Mapstacks' features in OS tests ###
#	Scenario: Check whether all map types are available
#		Given I log into OS maps as a guest/registered/subscribed user
#		When I click 'Change map' button
#		Then I should see all map types available
		
@SingleMap
	Scenario: Check whether only one map type is available-'Change Map Type' functionality is disabled
		Given I log into OS maps as a guest/registered/subscribed user
#		When only one map type is available
